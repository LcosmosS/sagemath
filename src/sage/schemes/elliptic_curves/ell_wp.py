...
def weierstrass_p(E, prec=20, algorithm=None):
    ...
    if algorithm == "pari":
        if 0 < p <= prec + 2:
            raise ValueError(f"for computing the Weierstrass p-function via pari, the characteristic ({p}) of the underlying field must be greater than prec + 2 = {prec + 2}")
    ...
    if algorithm == "quadratic":
        if 0 < p <= prec + 2:
            raise ValueError(f"for computing the Weierstrass p-function via the quadratic algorithm, the characteristic ({p}) of the underlying field must be greater than prec + 2 = {prec + 2}")
        wp = compute_wp_quadratic(k, A, B, prec)
    elif algorithm == "fast":
        if 0 < p <= prec + 4:
            raise ValueError(f"for computing the Weierstrass p-function via the fast algorithm, the characteristic ({p}) of the underlying field must be greater than prec + 4 = {prec + 4}")
